...
export default class MainScene extends Phaser.Scene {
  ...
  create() {
    const { width, height, buildings = [], houses = [], obstacles = [], zones = [] } = this.mapConfig;

    this.matter.world.setBounds(0, 0, width, height);
    this.cameras.main.setBounds(0, 0, width, height);

    // ---- Ground — one tinted rectangle + grid per zone, so crossing from
    // Old Meridian into Neo Meridian is felt immediately (a subtler, cooler
    // grid tone) without any loading screen or scene change. Falls back to
    // one uniform ground fill if the map config has no zones at all. ----
    if (zones.length > 0) {
      // Base fill first, covering the whole map including the empty buffer
      // strip between zones — then each zone's tint draws on top of it.
      this.add.rectangle(width / 2, height / 2, width, height, 0x1c2030);

      const ZONE_THEME = {
        old_meridian: { ground: 0x232a3f, grid: 0x2c3450, label: '#c7d2fe' },
        neo_meridian: { ground: 0x1a2233, grid: 0x2f5a6e, label: '#67e8f9' },
        dustridge_county: { ground: 0x2b2417, grid: 0x4a3f26, label: '#eab676' },
        port_haven: { ground: 0x1c2a33, grid: 0x2f5266, label: '#7dd3fc' },
        veltriz_sea: { ground: 0x0b2a3d, grid: 0x134e5e, label: '#5eead4' },
      };
      const DEFAULT_THEME = { ground: 0x232a3f, grid: 0x2c3450, label: '#c7d2fe' };

      zones.forEach((z) => {
        const theme = ZONE_THEME[z.key] || DEFAULT_THEME;
        const zoneWidth = z.maxX - z.minX;
        const centerX = z.minX + zoneWidth / 2;
        this.add.rectangle(centerX, height / 2, zoneWidth, height, theme.ground);
        this.add.grid(centerX, height / 2, zoneWidth, height, 64, 64, undefined, undefined, theme.grid, 0.4);

        this.add
          .text(z.minX + 40, 40, z.name.toUpperCase(), {
      ...
      // A thin "gateway" line marking the exact boundary between zones —
      // purely visual, no collision body, so walking through it is instant.
      for (let i = 1; i < zones.length; i++) {
        const gateX = (zones[i - 1].maxX + zones[i].minX) / 2;
        this.add.rectangle(gateX, height / 2, 6, height, 0x67e8f9, 0.25);
      }
    } else {
    ...
    // ---- Houses (visual + solid collision ONLY — no sensor/interaction
    // zone, deliberately. These are the procedurally-placed residential
    // buildings from worldData.js's placeHouses(); "Press E" does nothing
    // here on purpose until the Real Estate Agency makes one ownable — see
    // GamePage.jsx, which never lists 'home' with a houseType in its
    // openPanel map, so nothing happens even if a player somehow lingers.
    // Each house's own `color` gives it a visually distinct look from its
    // neighbors, per its houseType in the catalog. ----
    houses.forEach((h) => {
      const fillColor = h.color ? Phaser.Display.Color.HexStringToColor(h.color).color : 0x4a5170;
      this.add.rectangle(h.x, h.y, h.width, h.height, fillColor, 1).setStrokeStyle(2, 0x1c2038, 0.8);
      if (h.width >= 150) {
        this.add
          .text(h.x, h.y, h.name, {
            fontFamily: 'Segoe UI, Arial',
            ...
            color: '#e8eaff99',
            align: 'center',
            wordWrap: { width: h.width - 12 },
          })
          .setOrigin(0.5);
      }
      this.matter.add.rectangle(h.x, h.y, h.width, h.height, { isStatic: true, label: `solid:${h.id}` });
    });
  ...
  update(time) {
    ...
    // Broadcast positions to React for the minimap — throttled separately
    // from the (faster) socket movement emit since the minimap only needs
    // a few updates per second, not every frame.
    if (time - this.lastMinimapEmitAt > MINIMAP_EMIT_INTERVAL_MS) {
      this.lastMinimapEmitAt = time;
      gameEvents.emit('minimap:update', {
        self: { x: this.playerContainer.x, y: this.playerContainer.y },
        others: Array.from(this.remotePlayers.values()).map((entry) => ({
          x: entry.container.x,
          y: entry.container.y,
        })),
      });
