...
const buyItem = async (req, res, next) => {
  try {
    ...
    const totalCost = Math.round(found.currentPrice * quantity);
    ...
    try {
      wallet = await debitWallet(req.user.id, totalCost, {
        ...
        meta: { itemKey, quantity, unitPrice: found.currentPrice },
      });
    } catch (e) {
...
module.exports = { listMarket, getInventory, buyItem, sellItem };
